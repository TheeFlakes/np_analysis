<script>
	...
	let sidebarOpen = true; // Always open on desktop
...
<div class="flex h-screen bg-gray-100">
	...
	<aside
		class="hidden lg:flex bg-white border-r border-gray-200 transition-all duration-300 {sidebarOpen
			? 'w-64'
			: 'w-20'} relative h-screen z-40 flex-col"
	>
		<!-- Logo Section -->
		<div class="flex items-center justify-between h-20 px-4 border-b border-gray-200">
			...
			{:else}
				<div class="flex items-center justify-center w-full">
					<img src="/lea_page.jpg" alt="NP Millers Logo" class="w-10 h-10 object-contain" />
				</div>
			{/if}
		</div>
		...
		<button
			...
			class="absolute -right-3 top-20 bg-white border border-gray-200 rounded-full p-1 hover:bg-gray-50 flex items-center justify-center w-6 h-6 shadow-sm"
		>
			<svg
	...
	<nav class="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 shadow-lg safe-area-inset-bottom">
		<div class="flex items-center justify-around h-16 px-1">
			{#each menuItems as item}
				<a
					href={item.path}
					class="flex flex-col items-center justify-center flex-1 h-full rounded-lg transition-colors min-w-0 px-1 {$page.url.pathname === item.path
						? 'text-blue-600 bg-blue-50'
						: 'text-gray-600'}"
				>
					<!-- Dashboard Icon -->
					{#if item.icon === 'dashboard'}
						<svg class="w-5 h-5 mb-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							<path
								stroke-linecap="round"
								stroke-linejoin="round"
								stroke-width="2"
					...
					<span class="text-[10px] font-medium truncate w-full text-center leading-tight">{$page.url.pathname === item.path ? item.name : item.name}</span>
				</a>
			{/each}
		</div>
	</nav>
