...
<div class="min-h-screen bg-gray-50 p-3 sm:p-6">
	<div class="max-w-7xl mx-auto">
		...
		<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 sm:mb-8">
			<div>
				...
				<p class="text-sm sm:text-base text-gray-600 mt-1">Manage all your customers in one place</p>
			</div>
		...
		{:else}
			...
			<div class="md:hidden space-y-4">
				{#each filteredCustomers as customer (customer.id)}
					<div class="bg-white rounded-lg shadow-sm p-4 border border-gray-200" on:click={() => viewCustomerDetails(customer)}>
						<div class="flex items-start justify-between mb-3">
							<div class="flex-1">
								<h3 class="text-base font-semibold text-gray-900 mb-1">{customer.name}</h3>
								<p class="text-sm text-gray-600">{customer.region || 'No region'}</p>
							</div>
						...
						<div class="flex gap-2 pt-3 border-t border-gray-200" on:click|stopPropagation>
							<button
								on:click={() => openEditModal(customer)}
	...
	{#if showEditModal}
		<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4">
			<div class="bg-white rounded-lg shadow-lg max-w-md w-full max-h-[95vh] overflow-y-auto">
				...
				<form on:submit|preventDefault={updateCustomer} class="p-4 sm:p-6 space-y-4">
					<div>
						<label class="block text-sm font-medium text-gray-900 mb-1">Name *</label>
						<input
							type="text"
	...
	{#if showDetailCard && selectedCustomer}
		<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 overflow-y-auto">
			<div class="bg-white rounded-lg shadow-2xl max-w-5xl w-full my-2 max-h-[95vh] overflow-y-auto">
				<!-- Hero Header -->
